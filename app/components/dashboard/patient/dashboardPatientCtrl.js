angular.module('app.components.dashboard.patient', []).controller('dashboardPatientCtrl',
    ['$scope', 'ngDialog', 'accountResource', 'accountService', '$location', '$routeParams', 'patientProfileResources', 'patientProfileMeasurementResource', 'observationResource', 'appointmentResources',
        function ($scope, ngDialog, accountResource, accountService, $location, $routeParams, patientProfileResources, patientProfileMeasurementResource, observationResource, appointmentResources) {
            ...
            $scope.toAppo = function () {
                $location.path($location.$$path + '/appointment');
            };
            ...
            $scope.getOldObservations = function (number) {
                observationResource().getOldObservations({
                    patientId: $routeParams.patientId,
                    number: number
                }).$promise.then(function (response) {
            ...
            $scope.getDiseases = function (number) {
                observationResource().getDiseases({
                    patientId: $routeParams.patientId,
                    number: number
                }).$promise.then(function (response) {
            ...
            $scope.getMedications = function (number) {
                observationResource().getMedications({
                    patientId: $routeParams.patientId,
                    number: number
                }).$promise.then(function (response) {
            ...
            $scope.getUpcomingAppointments = function (number) {
                appointmentResources().getUpcomingAppointmentForPatientProfile({
                    patientId: $routeParams.patientId,
                    number: number
                }).$promise.then(function (response) {
                    ...
                    if (number != -1)
                        $scope.upcomingAppointments.opened = false;
                    else
                        $scope.upcomingAppointments.opened = true;
                }, function (response) {
                    $scope.upcomingAppointments = [];
                    if (number != -1)
                        $scope.upcomingAppointments.opened = false;
            ...
            $scope.getMeasurements = function (number) {
                observationResource().getMeasurements({
                    patientId: $routeParams.patientId,
                    number: number
                }).$promise.then(function (response) {
            ...
            $scope.getUpcomingAppointments(5);
